...
class NoticiasController extends Controller
{
    ...
    if (file_exists($path)) {
        ...
        foreach ($newsData->items as $value) {
            if ($i <= 8) { // Mantenemos tu límite original
                // IMPORTANTE: Usamos los nombres que vienen en tu nuevo JSON
                $noticias[] = array(
                    'titulo'      => $value->titulo,
                    'link'        => $value->link,
                    'description' => $value->description,
                    'img'         => $value->img ?: 'https://static.vecteezy.com/system/resources/previews/004/141/669/non_2x/no-photo-or-blank-image-icon-loading-images-or-missing-image-mark-image-not-available-or-image-coming-soon-sign-simple-nature-silhouette-in-frame-isolated-on-blue-background-free-vector.jpg',
                    'fecha'       => $value->fecha, // Ya viene formateada del script
