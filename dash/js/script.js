		...
		function initLightGallery(itemsToInit, addClass) {
			if (!isNoviBuilder) {
				$(itemsToInit).lightGallery({
					...
					pause:     parseInt($(itemsToInit).attr("data-lg-autoplay-delay")) || 1900,
					addClass:  addClass,
		...
		function scrollFunction() {
			...
			if ($(window).width() > 820) {  
				...
				if (document.body.scrollTop > 0 || document.documentElement.scrollTop > 145.806) {
					...
					$('#fixlogo').attr('src',flagsUrl + 'logo_1.png');
					$('.text-menu a').css('color','#08475A');
					$('.menu-hamburg').css('background-color','#08475A');
			
				}
				else {
					$('#fixlogo').attr('src', flagsUrl + 'logo_footer.png');
					$('.text-menu a').css('color','#fff');
					$('.menu-hamburg').css('background-color', 'transparent');
					$('.rd-dropdown-link').css('color','rgb(8, 71, 90)');
					$('.rd-megamenu-list-link').css('color','rgb(8, 71, 90)');
